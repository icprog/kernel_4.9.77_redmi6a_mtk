...
//drivers\watchdog\mediatek\include
...
#define ESDCHECK_CYCLE_TIME             2000    //ms
...
static int mxt_proc_message(struct mxt_data *data, u8 *message)
{
	...
	if (report_id == data->T6_reportid) {
		...
		//printk("mxt: mxt_proc_message, report_id = %d,\n",report_id);
...
static int touch_event_handler(void *pdata)
{
	...
	if (!data->object_table) {
		...
		if(!mxt_esdcheck_data.esd_detect_cnt)  
			mxt_esdcheck_data.esd_detect_cnt=1;  //for other interrupt message, also set the flag to prevent false reset if having touch on TP.
...
/*
	if (strncmp(mxt_cfg_data, MXT_CFG_MAGIC, strlen(MXT_CFG_MAGIC))) {
	...
	*/
	...
	for(i=0;i<3;i++){
		...
		//ret = sscanf(mxt_cfg_data + data_pos, "%x%n", &info_crc_tmp[i], &offset);
	...
	/*
	ret = sscanf(mxt_cfg_data + data_pos, "%x%n", &info_crc, &offset);
	if (ret != 1) {
		dev_err(dev, "Bad format: failed to parse Info CRC\n");
		ret = -EINVAL;
		goto release;
...
*/
	...
	for(i=0;i<3;i++){
		...
		/*
		ret = sscanf(mxt_cfg_data + data_pos, "%x%n", &cfg_crc_tmp[i], &offset);
		if (ret != 1) {
	...
	/*
	ret = sscanf(mxt_cfg_data + data_pos, "%x%n", &config_crc, &offset);
	if (ret != 1) {
		dev_err(dev, "Bad format: failed to parse Config CRC\n");
		ret = -EINVAL;
		goto release;
	...
	*/
	...
	while (data_pos < len) {
		...
		//ret = sscanf(mxt_cfg_data + data_pos, "%x %x %x %x %x %x%n",
		//	     &type_tmp[0], &type_tmp[1],&instance_tmp[0], &instance_tmp[1], &size_tmp[0], &size_tmp[1], &offset);
...
static u8 mxt_read_chg(struct mxt_data *data)
{
	...
	//printk("mxt : mxt_read_chg gpio_irq num :%ld val:%d \n",data->pdata->gpio_irq,val);
					...
					const char *buf, size_t count)
{
	...
	// update fw name to data->fw_name
...
static int mxt_load_fw_data(struct device *dev)
{
	...
	while (pos < len) {
        ...
        //printk("mxt: load fw: i = pos = %d, frmae_size=%d\n", pos, frame_size);
		...
		if (frame % 10 == 0){
			...
			frame, pos, len);	// fw->size;
			//msleep(50);
			mtk_wdt_restart(WD_TYPE_NORMAL);//(WD_TYPE_NORMAL);
		}
    	}
		...
		struct device_node *np, struct mxt_config_info *info)
{
	  ...
	  printk("mxt_parse_config_dt start\n");
					...
					const char *buf, size_t count)
{
	...
	if (error)
		return error;
	//check if the fw CRC stored on the cfg file is same with the chip's
		...
		const char *buf, size_t count)
{
	...
	//TPD_DMESG("mxt_update_cfg_store %d, %d\n",__LINE__, mxt_touch_irq);
...
{	
	...
	//get the default control register value for writing back after operation
	...
	// disable mutual p2p scan for getting the raw data
...
//	printk("ttt7 t37_buf addr:%p \n",(void *)data->T37_buf); 
	...
	//write back the default control register value
...
static struct mxt_platform_data *mxt_parse_dt(struct i2c_client *client)
{
	...
	pdata->gpio_irq = 0; //davaid add for get interrupt pin directtly
	...
	struct mxt_config_info *info, bool force)
{
	...
	//no need to read from DTS since the fine name has been updated 
	...
	//if fw is updated, then update cfg anyway!
	if (force){
	...
	//judge the config array format, if the first object is T37
	t37 = mxt_cfg_data[MXT_CFG_ARRAY_INFO_LENGTH + 2];
	
	//if the first object is T37, re-shift the offset of the config id to add the T37 data.
	if(t37 == 0x25){
	...
	//if no T37 object, get the cfg_id driectly.
	else if(t37==0x44)
	{
	...
	//read the config id from the chip.
	...
	// if the config file has early config id then will not update
	...
	*ppos += count;
	...
	for (ii = 0; ii < TX_NUM; ii++) {
		for (jj = 0; jj < RX_NUM; jj++) {
			...
			report_data_16++;
			//printk("ii:%d ,jj:%d ,limit_b:%d ,limit_l:%d , report_data_16:%d \n",ii,jj,limit_b,limit_l,*report_data_16);
		}
	}
...
static int fw_upgrade_handler(void *pdata)
{
	...
	//Step:5 Create sysfs bin files for BIST
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//Step3: read info and object table
    ...
    //Step:4 Auto upgrading FW/Config if needed.
    /* - added by david */
...
void mxt_set_gesture(struct mxt_data *mxt, bool enable)
{
	...
	if (enable) {
		...
		//mxt_config_ctrl_set(mxt, mxt->T115_address, 0x01);
	} else {
		...
		//mxt_config_ctrl_clear(mxt, mxt->T115_address, 0x01);
	}
